...
private:
    ...
    Packet(std::array<uint8_t, 2> id, std::vector<uint8_t> data_buffer);
    Packet(uint8_t const* packet_buffer, std::size_t size);

    uint8_t compute_crc() const;
    void write_crc();
    void write_header(std::array<uint8_t, 2> id);
...
public:
    ...
    static Packet construct_with_id(std::array<uint8_t, 2> id, std::vector<uint8_t> data_buffer); 

    static constexpr std::size_t CRC_INDEX = 2;
    static constexpr std::size_t HEADER_SIZE = sizeof(uint8_t) * 4;

    std::array<uint8_t, 2> get_id() const;
